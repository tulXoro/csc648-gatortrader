<script lang="ts">
  ...
  import img from "$lib/assets/no_image.jpg";
...
>
  <div class="p-4 bg-slate-200">
    <a href={`/viewPost/${post.post_id}`} target="_blank">
      ...
      {:else}
        <!-- Use a stock image -->
        <img class="object-cover w-full h-64" src={img} alt="Stock Image" />
      {/if}
    </a>
    <a
      target="_blank"
      class="no-underline hover:underline border-gray-300 text-orange-600 hover:text-orange-800"
    ...
    <p
      class="text-md font-semibold mb-4 text-ellipsis overflow-hidden"
